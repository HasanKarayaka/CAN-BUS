...
//uint32_t RxData[8];
//uint32_t TxMailBox;
unsigned char amp1_buff1[1];
unsigned char amp1_buff2[1];

typedef struct {
    CAN_TxHeaderTypeDef TxHeader; // Mesaj başlığı
    CAN_RxHeaderTypeDef RxHeader;

    unsigned char TxData[1];            // Veri
    uint32_t TxMailbox;           // Mailbox ID
    uint8_t RxData[5];            // Alınan veri
    unsigned char feedback_amp;        // Geri bildirim verisi
} CAN_MessageTypeDef;
